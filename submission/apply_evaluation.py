...
Modified version from the code of the Learn2Reg organisers (Adrian Dalca, Alessa Hering, Lasse Hansen and Mattias Heinrich)
The original code is available here : https://learn2reg.grand-challenge.org/Submission/
This code was to modified to applied the evaluation metrics only to the task4 and to adapt input path
"""
...
def main():
    ...
    DEFAULT_GROUND_TRUTH_PATH = data_path + 'L2R_Task4_HippocampusMRI/Training/label/'
    submission_name = sys.argv[1] 
    DEFAULT_INPUT_PATH = submission_path + submission_name + '/task_04/'
    pairs_path = './datasets/task_04_pairs_val.csv'
...
class EvalVal():
    ...
    def evaluate(self):
        ...
        self.load_task_04()
        print(self.disp_fields_task_04)
    ...
    def load_task_04(self):
        self.pairs_task_04 = self.load_pairs(self.pairs_path)
        self.segs_task_04 = self.load_segs_task_04()
        self.disp_fields_task_04 = self.load_disp_fields(self.pairs_task_04, self.DEFAULT_INPUT_PATH, np.array([3, 64, 64, 64]))
    ...
    def load_segs_task_04(self):
        ...
        for i in np.unique(indices):
            case = self.nifti_loader.load(fname=self.DEFAULT_GROUND_TRUTH_PATH + 'hippocampus_{:03d}.nii.gz'.format(i))
