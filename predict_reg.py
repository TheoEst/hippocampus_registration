# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 16:12:58 2020
...
def parse_args():
    ...
    parser.add_argument('--nb-batch', type=int, default=9,
                        help='Number of batch to plot')

    parser.add_argument('--all-dataset', action='store_true',
                        help='Use the all dataset')
    ...
    parser.add_argument('--save-submission', action='store_true', 
                        default=False, help='Save the submission for leaderboard')
...
def predict(args):
    ...
    args.submission_path = args.save_path + 'submission/' + model_name + '/task_04/'
...
def save_pred(deformed_img, deformable_grid, integrated_grid,
    ...
    deformed_img = utils.to_numpy(args, deformed_img).squeeze()
    deformed_mask = utils.to_numpy(args, deformed_mask).squeeze()
    deformed_mask = np.argmax(deformed_mask, axis=0)
    
    data_path = main_path + 'data/L2R_Task4_HippocampusMRI/'
    data_path += 'Testing/' if args.test else 'Training/'
    
    (moving_patient, 
    reference_patient) = int(moving_patient), int(reference_patient)
    irm_path = data_path + 'img_pad/hippocampus_{:03d}.nii.gz'.format(moving_patient)
    
    if args.save_deformed_img:
        
        sitk_img = sitk.ReadImage(irm_path)
        
        deformed_mask = numpy2nifty.numpy2nifty(deformed_mask, sitk_img)
        deformed_img = numpy2nifty.numpy2nifty(deformed_img, sitk_img)
        
        pred_path = args.pred_path + str(reference_patient) + '_' + str(moving_patient)
    ...
    integrated_grid = utils.to_numpy(args, integrated_grid).squeeze()   
            
    if args.save_grid:
        deformable_grid = utils.to_numpy(args, deformable_grid).squeeze()
     
        for i, x in enumerate(['x', 'y', 'z']):
            
            path = pred_path + '_deformed_grid_' + x + '.nii.gz'
            grid_x = numpy2nifty.numpy2nifty(deformable_grid[i, ...], sitk_img)
            sitk.WriteImage(grid_x, path)
        
            path = pred_path + '_integrated_grid_' + x + '.nii.gz'
            grid_x = numpy2nifty.numpy2nifty(integrated_grid[i, ...], sitk_img)
            sitk.WriteImage(grid_x, path)
        
    if args.save_submission:
        scipy_grid = convert_pytorch_grid2scipy(integrated_grid)
        path = args.submission_path + 'disp_{:04d}_{:04d}.npy'.format(reference_patient, moving_patient)
